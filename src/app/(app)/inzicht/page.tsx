...
export default async function GraafPage({
  ...
  return (
    <>
      ...
      <h1 className="screen-title">{q ? `Waar hangt ${q} mee samen?` : "Vraag je verbindingen"}</h1>
              ...
              <li key={s}>
                <div className="row">
                  <div className="row__body">
                    <Link href={`?q=${encodeURIComponent(s)}`} className="row__title">
                      {s}
                    </Link>
                  </div>
                </div>
        ...
        <section className="section">
          ...
          {result.nodes.length === 0 ? (
            <div className="empty">
              ...
              De datakoppeling kent dit begrip nog niet. Probeer een naam van een persoon, project of
              organisatie.
            </div>
          ) : (
            <>
      ...
      <p className="meta" style={{ marginTop: "var(--s-6)" }}>
        De data staat per gebruiker apart, in de EU, versleuteld. Er wordt niet op getraind.
      </p>
    </>
  );
}
